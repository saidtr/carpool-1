...
class View_RegistrationMail {
    ...
    public static function render($contact) {
        ...
        $html = '<html>' .
                ...
                '</style>' .
                '<body>' .
                '<h1>' . sprintf(_('Thanks, %s'), htmlspecialchars($contact['Name'])) . '!</h1>' .
                '<div id="content">' .
                '<p>' . _('You sucssfully joined the carpool.') . '</p>' .
				'<p>' . sprintf(_('You can always update or delete your account by browsing to %s'), '<a href="' . htmlspecialchars($authUrl) . '">' . htmlspecialchars($authUrl) . '</a>') . '.</p>' .
        		'<p>' . _('Unless you ask for it, you will never get any more emails from this site.') . '</p>' .
                '<p>' . _('Thanks') . ',<br/>' . sprintf('The %s team', _(getConfiguration('app.name'))) .
				'</div>' .
                '</body>' .
                '</html>';        
        return $html;      
